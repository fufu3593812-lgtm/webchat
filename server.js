...
// 8/19: 服服×Ice 聊天前端(SSE 实时/世界书/记忆/上下文/工具)
...
const CLIENT_PENDING_FILE = "./client_pending.json";
...
// ========================
// 给 timeline 中缺少时间戳前缀的 user 消息补时间戳 (wake_up 依赖 content 开头的 YYYY-MM-DD HH:mm)
// ========================
function pad2(n) { return String(n).padStart(2, "0"); }
function localTimeString(d) {
  return d.getFullYear() + "-" + pad2(d.getMonth() + 1) + "-" + pad2(d.getDate()) + " " + pad2(d.getHours()) + ":" + pad2(d.getMinutes());
}
function stampTimeline(messages, tsDB) {
  return messages.map(function (msg) {
    if (msg.role !== "user") return msg;
    if (typeof msg.content !== "string") return msg;
    const content = msg.content.trim();
    if (/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}/.test(content)) return msg;
    const t = extractTimestampWithMemory(msg, tsDB) || new Date();
    return Object.assign({}, msg, { content: localTimeString(t) + " " + content });
  });
}
...
// ========================
// 客户端待展示消息队列
// wake_up 推送成功后把消息体 POST 到 /internal/client-message 排队;
// 下次 /v1 请求流式返回前, 以独立 assistant 气泡注入客户端。
// ========================
let clientPending = [];
function loadClientPending() {
  if (!fs.existsSync(CLIENT_PENDING_FILE)) return [];
  try {
    const arr = fs.readJsonSync(CLIENT_PENDING_FILE);
    return Array.isArray(arr) ? arr : [];
  } catch (e) {
    return [];
  }
}
clientPending = loadClientPending();
function saveClientPending() {
  try { fs.writeJsonSync(CLIENT_PENDING_FILE, clientPending, { spaces: 2 }); } catch (e) {}
}
...
function appendSpecialEvent(content) {
  ...
  // 8/30: 加 ts 字段——wake daemon 喂历史时按 ts 带时间(没有 ts 的老记录会被当成"刚发生" → 时间错乱)
  const newEvent = { role: "assistant", content, position: maxPos + 0.5, ts: localTimeString(new Date()) };
...
app.addHook("onRequest", (req, reply, done) => {
  // 8/19: 聊天前端(chat.js)路由放行; 8/29: /dock/ = 底部 dock 图标
  if (req.url === "/chat" || req.url.startsWith("/chat/") || req.url.startsWith("/api/chat/") || req.url.startsWith("/dock/")) return done();
  if (req.url.startsWith("/admin")) return done();
  // 批注 2026-07-11：本地部署默认只允许局域网；Railway/Render 等云端部署需显式打开公网 /v1 API，内部接口仍不外放。
  if (req.url.startsWith("/v1/")) {
    // 入站鉴权(加固):配置了 GATEWAY_API_KEY 时要求 Bearer / x-api-key 匹配(Kelivo 填的 API Key)
    const expected = readEnvValue("GATEWAY_API_KEY");
    if (expected) {
      const auth = req.headers.authorization || "";
      const xkey = req.headers["x-api-key"] || "";
      const ok = auth === "Bearer " + expected || auth === expected || xkey === expected;
      if (!ok) return reply.code(401).send("Unauthorized");
    }
    if (readBooleanEnv("ALLOW_PUBLIC_API", false)) return done();
  }
...
app.get("/v1/models", async (req, reply) => {
  reply.send({
    object: "list",
    data: [{ id: readEnvValue("MODEL_NAME") || "DeepSeek-V4-Pro", object: "model", created: 0, owned_by: "gateway" }]
  });
});
...
app.post("/v1/chat/completions", async (req, reply) => {
  try {
    ...
    for (const msg of kelivoMessages) {
      ...
      // 2026-08-18 唤醒修复: content 无内嵌时间戳时用到达时间兜底, 保证 tsDB 对每条消息都有记录
      const ts = extractTimestamp(normalizeContentToText(msg.content)) || new Date();
      const fp = makeFingerprint(msg);
      const fpStripped = makeFingerprintStripped(msg);
    ...
    saveTimeline(stampTimeline(finalTimeline, tsDB));
    ...
    const response = await fetch(TARGET_API_URL, {
      ...
      body: JSON.stringify({ ...body, model: readEnvValue("MODEL_NAME") || body.model, messages: llmMessages })
    });
    ...
    if (!shouldStreamResponse) {
      ...
      // 非流式兜底：把待展示消息合并进回复文本(客户端默认走流式, 这里是防御)
      if (clientPending.length > 0 && response.status === 200) {
        try {
          const parsed = JSON.parse(responseText);
          if (parsed.choices && parsed.choices[0] && parsed.choices[0].message) {
            const current = String(parsed.choices[0].message.content || "");
            const pendingTexts = clientPending
              .map(function (p) { return "【唤醒】" + p.content; })
              .join("\n\n");
            parsed.choices[0].message.content = pendingTexts
    ...
    // 注入待展示的唤醒消息为独立 assistant 气泡(SSE 完整一轮: role→content→stop),
    // 再接上游真实流; 故意不插 [DONE], 避免客户端收到后提前关闭、丢弃真实回复。
    const modelLabel = readEnvValue("MODEL_NAME") || "assistant";
    const nowSec = Math.floor(Date.now() / 1000);
    if (clientPending.length > 0) {
      const pendingToEmit = clientPending.slice();
      for (let i = 0; i < pendingToEmit.length; i++) {
        const id = "chatcmpl-client-" + i + "-" + nowSec;
        const base = { id, object: "chat.completion.chunk", created: nowSec, model: modelLabel, choices: [{ index: 0, delta: {}, finish_reason: null }] };
        reply.raw.write("data: " + JSON.stringify(Object.assign({}, base, { choices: [{ index: 0, delta: { role: "assistant", content: "" }, finish_reason: null }] })) + "\n\n");
        reply.raw.write("data: " + JSON.stringify(Object.assign({}, base, { choices: [{ index: 0, delta: { content: pendingToEmit[i].content }, finish_reason: null }] })) + "\n\n");
        reply.raw.write("data: " + JSON.stringify(Object.assign({}, base, { choices: [{ index: 0, delta: {}, finish_reason: "stop" }] })) + "\n\n");
      }
      clientPending = [];
      saveClientPending();
    }
...
// ========================
// 内部接口：wake_up 推送成功后, 把消息体交给网关排队,
// 供客户端下次 /v1 请求时以独立气泡展示。
// ========================
app.post("/internal/client-message", async (req, reply) => {
  try {
    const { content, contact, window } = req.body;
    if (!content || typeof content !== "string") {
      return reply.code(400).send({ error: "content is required" });
    }
    const text = content.trim();
    if (!text) return reply.code(400).send({ error: "content is empty" });
    clientPending.push({ content: text, time: localTimeString(new Date()) });
    saveClientPending();
        // 8/19: 同步到网页聊天(chat.js 的 onWake 追加 webchat.json + SSE 广播)
        // 8/30 换窗: contact/window 透传, 唤醒气泡落指定窗口(缺省落活动窗口)
        try { chat.onWake({ content: text, contact, window }); } catch (e) { console.error("chat.onWake failed:", e.message); }
    reply.send({ success: true, queued: clientPending.length });
  } catch (err) {
    console.error(err);
    reply.code(500).send({ error: err.message });
  }
...
// ========================
// 内部接口：唤醒工具(供 wake_up.js 调用)
// 网关绑定 0.0.0.0 公网可达, 而这里是执行 Ice 的工具(能写日记/发音乐/问服服等),
// 必须用共享密钥 X-Internal-Key(= .env 的 INTERNAL_KEY) 守卫, 与 wake_up.js 同读一个 .env。
// ========================
function wakeInternalKey(req, reply, done) {
  const key = readEnvValue("INTERNAL_KEY");
  if (!key) {
    reply.code(503).send({ error: "INTERNAL_KEY 未配置" });
    return;
  }
  const provided = String(req.headers["x-internal-key"] || req.headers["x-internal-key-2"] || "");
  if (provided !== key) {
...
// GET: 给唤醒进程一份当前联系人的工具清单(与聊天同源, 含 MCP 展开)
app.get("/internal/wake-tools", { preHandler: wakeInternalKey }, async (req, reply) => {
  try {
    const c = chat.getContact();
    const tools = await chat.buildToolDefs(c);
    reply.send({ tools: tools || [] });
  } catch (err) {
    console.error(err);
    reply.code(500).send({ error: err.message });
  }
...
// POST: 执行一个工具调用(唤醒进程没带联系人配置/MCP, 统一经网关执行)
app.post("/internal/wake-tool", { preHandler: wakeInternalKey }, async (req, reply) => {
  try {
    ...
    const c = chat.getContact();
    const tc = { id: "wake-" + Date.now(), type: "function", function: { name, arguments: JSON.stringify(args ?? {}) } };
    const result = await chat.runTool(c, tc);
    reply.send({ result });
  } catch (err) {
    console.error(err);
    reply.code(500).send({ error: err.message });
...
// 8/30 换窗: 唤醒守护取当前活动窗口(服服正在看的窗口), 推送气泡要落这里
app.get("/internal/active-window", { preHandler: wakeInternalKey }, async (req, reply) => {
  try {
    const cfg = chat.config || {};
    const contact = String(cfg.activeContactId || "ice");
    const window = String((cfg.activeWindow && cfg.activeWindow[contact]) || "0");
    reply.send({ contact, window });
  } catch (err) {
    console.error(err);
    reply.code(500).send({ error: err.message });
  }
...
// 8/31 唤醒搬进网关后: 手动触发一次唤醒(用于人工验证 / 运维兜底)。doWake 自带冷却逻辑,
// 但主动调用仍执行生成+落库+广播+Bark(跳过 shouldWake 空闲门槛, 测试/应急直接唤一次)。
app.post("/internal/wake-now", { preHandler: wakeInternalKey }, async (req, reply) => {
  try {
    const force = !!(req.body && req.body.force);
    if (!force && !chat.shouldWake()) {
      return reply.send({ ok: false, skipped: true, reason: "notDueYet" });
    }
    const r = await chat.doWake();
    reply.send({ ok: true, result: r });
  } catch (err) {
    console.error(err);
    reply.code(500).send({ error: err.message });
  }
...
// 8/19: 注册聊天前端路由
chat.register(app);
// 8/31: 唤醒搬进网关 —— 调度器在 register 之后启动(不进 register, Fastify 测试不留悬挂定时器)
chat.startWakeScheduler();
