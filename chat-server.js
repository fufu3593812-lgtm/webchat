...
async function runChat(c, msgsCid, opts, onDelta) {
  ...
  for (let i = 0; i < c.context.toolIterations; i++) {
    ...
    accContent += (data.content || "");   // 本轮 content 已通过 onDelta 实时流给客户端, 必须累积, 否则最终消息比 live 少中间话(「回复完就变了」)
...
function register(app) {
  ...
  // 删除消息(单条 id 或批量 ids): 单聊走 winKey(绝不用裸 c.id, 防裸 key 覆盖真窗口); 群聊用裸 g.id(群 key 本就是裸 id)
